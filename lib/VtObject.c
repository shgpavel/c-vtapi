...
This is a library to implement API calls for VirusTotal's
http://www.virustotal.com/ public and private API's.
...
Newly created objects have a count of one.  Use the ObjectName_get() function to
get another reference. Use ObjectName_put() to release the reference. When the
counter to the reference reaches zero the object is freed with free()
...
If you are just a user of this library and not developing the library don't
worry about this.


*/

/// VtObject ID sequence.  NOTE  object allocation for now only done by one
/// thread so no mutex needed yet.
...
void VtObject_register(struct VtObject_ops *ops) {
	LOCK_MUTEX(&mutex);
	obj_types_list = realloc(obj_types_list, sizeof(struct VtObject_ops *) *
	                                             (num_obj_types + 2));
...
struct VtObject *VtObject_alloc(struct VtObject_ops *ops) {
	...
	int retval = 0;
	if (ops->obj_size < sizeof(struct VtObject)) BUG();
 ...
 * @brief Note this should only be called from VtObject_put() or an inherited
 * _put()
 * @arg obj  pointer to pointer to object so we can return NULL pointer of
 * free'd memory
 */
void VtObject_free(struct VtObject **obj) {
	...
	if (ops->obj_destructor) ops->obj_destructor(*obj);
...
void VtObject_put(struct VtObject **obj_arg) {
	...
	obj->refcount--;
	DBG(DGB_LEVEL_MEM,
	    "Returned object reference %p, %d remaining type: %s\n", obj,
	    obj->refcount, obj->obj_ops->obj_type);

	if (obj->refcount < 0) {
		VT_ERROR_FATAL("Refcount = %d \n", obj->refcount);
	}

	if (obj->refcount <= 0) VtObject_free(obj_arg);
...
static void CCALL VtObject_exit(void) {
	if (obj_types_list) free(obj_types_list);
}
