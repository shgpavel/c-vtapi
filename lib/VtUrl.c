...
int VtUrl_scan(struct VtUrl *vt_url, const char *url) {
	...
	if (ret) VT_ERROR("Adding url %s\n", url);
	ret = curl_mime_name(part, "url");
	if (ret) VT_ERROR("Adding url multipart name %s\n", url);
...
int VtUrl_report(struct VtUrl *vt_url, const char *resource, bool scan,
                 bool all_info) {
	CURL *curl = NULL;
	CURLcode res;
	int ret = 0;
	...
	if (ret) VT_ERROR("Adding resource multipart name %s\n", resource);
	...
	if (scan) {
		...
		if (ret) VT_ERROR("Adding scan\n");
		ret = curl_mime_name(part, "scan");
		if (ret) VT_ERROR("Adding scan multipart name\n");
	}

	if (all_info) {
		part = curl_mime_addpart(mime);
		ret = curl_mime_data(part, "1", CURL_ZERO_TERMINATED);
		if (ret) VT_ERROR("Adding all_info\n");
		ret = curl_mime_name(part, "all_info");
		if (ret) VT_ERROR("Adding all_info multipart name\n");
	}
