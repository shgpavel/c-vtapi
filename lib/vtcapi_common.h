...
#define DBG(LVL, FMT, ARG...)                                       \
	if (LVL <= DEBUG_LEVEL) {                                   \
		fprintf(stderr, PROG_NAME "<" #LVL ">:%s:%d: " FMT, \
...
#define DBG(LVL, FMT, ...)                                         \
	if (LVL <= debug_level) {                                  \
		fprintf(stderr, PROG_NAME "<%d>:%s:%d: " FMT, LVL, \
...
#define CRIT(FMT, ...)                                             \
	{                                                          \
		fprintf(stderr, PROG_NAME " Critical:%s:%d: " FMT, \
		        __FUNCTION__, __LINE__, __VA_ARGS__);      \
	}
#else
#define CRIT(FMT, ARG...)                                          \
...
#define VT_ERROR(FMT, ...)                                                    \
	{                                                                     \
		fprintf(stderr, PROG_NAME " ERROR:%s:%d: " FMT, __FUNCTION__, \
...
#define WARN(FMT, ...)                                            \
	{                                                         \
		fprintf(stderr, PROG_NAME " Warning:%s:%d: " FMT, \
		        __FUNCTION__, __LINE__, __VA_ARGS__);     \
	}
#else
#define WARN(FMT, ARG...)                                         \
...
#define INFO(FMT, ...)                                                       \
	{                                                                    \
		fprintf(stderr, PROG_NAME " Info:%s:%d: " FMT, __FUNCTION__, \
		        __LINE__, __VA_ARGS__);                              \
	}
#else
#define INFO(FMT, ARG...)                                                    \
...
#ifdef _MSC_VER
#define VT_ERROR_FATAL(FMT, ...)        \
	{                               \
		CRIT(FMT, __VA_ARGS__); \
		BUG();                  \
	}
#else
#define VT_ERROR_FATAL(FMT, ARG...) \
	{                           \
		CRIT(FMT, ##ARG);   \
		BUG();              \
	}
...
#ifdef _MSC_VER
#define snprintf(buff, sz, FMT, ...) \
	_snprintf_s(buff, sz, _TRUNCATE, FMT, __VA_ARGS__)
#else
#define snprintf(buff, sz, FMT, ARG...) \
	_snprintf_s(buff, sz, _TRUNCATE, FMT, ##ARG)
#endif
#endif
