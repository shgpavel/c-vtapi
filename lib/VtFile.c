...
int VtFile_destructor(struct VtObject *obj) {
	...
	if (file_scan->offset) free(file_scan->offset);
...
static int xferinfo(void *p, curl_off_t dltotal, curl_off_t dlnow,
	...
	if (file->cancel_operation) return 1;

	return 0;
}

/* for libcurl older than 7.32.0 (CURLOPT_XFERINFOFUNCTION) */
static int older_progress(void *p, double dltotal, double dlnow, double ultotal,
                          double ulnow) {
...
static void set_std_curl_data(struct VtFile *file, CURL *curl) {
	...
	curl_easy_setopt(curl, CURLOPT_SSL_VERIFYPEER,
	                 0L);  // disable validation
	curl_easy_setopt(curl, CURLOPT_SSL_VERIFYHOST, 0L);
#endif

	/* enable verbose for easier tracing */
	if (debug_level) curl_easy_setopt(curl, CURLOPT_VERBOSE, 1L);

	curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION,
	                 __VtApiPage_WriteCb);            // callback for data
...
int VtFile_scan(struct VtFile *file_scan, const char *file_path,
	...
	if (ret) VT_ERROR("Adding file %s\n", file_path);
	ret = curl_mime_name(part, "file");
	if (ret) VT_ERROR("Adding file multipart name %s\n", file_path);

	/* Fill in the filename field */
	part = curl_mime_addpart(mime);
	ret = curl_mime_data(part, file_path, CURL_ZERO_TERMINATED);
	if (ret) VT_ERROR("Adding filename %s\n", file_path);
	ret = curl_mime_name(part, "filename");
	if (ret) VT_ERROR("Adding filename multipart name %s\n", file_path);

	if (notify_url && notify_url[0]) {
		part = curl_mime_addpart(mime);
		ret = curl_mime_data(part, notify_url, CURL_ZERO_TERMINATED);
		if (ret) VT_ERROR("Adding notify_url %s\n", notify_url);
	...
	if (ret) VT_ERROR("Adding key\n");
	ret = curl_mime_name(part, "apikey");
	if (ret) VT_ERROR("Adding key multipart name\n");
	...
	if (file_scan->response) VtResponse_put(&file_scan->response);
...
cleanup:
	...
	if (headerlist) curl_slist_free_all(headerlist);  // free headers
...
int VtFile_scanMemBuf(struct VtFile *file_scan, const char *filename,
	...
	if (ret) VT_ERROR("Adding file memory buffer\n");

	if (notify_url && notify_url[0]) {
		part = curl_mime_addpart(mime);
		ret = curl_mime_data(part, notify_url, CURL_ZERO_TERMINATED);
		if (ret) VT_ERROR("Adding notify_url value\n");
		ret = curl_mime_name(part, "notify_url");

		if (ret) VT_ERROR("Adding notify_url name\n");
	}
	part = curl_mime_addpart(mime);
	ret = curl_mime_data(part, file_scan->api_key, CURL_ZERO_TERMINATED);
	if (ret) VT_ERROR("Adding key value\n");
	ret = curl_mime_name(part, "apikey");

	if (ret) VT_ERROR("Adding key name\n");
...
int VtFile_rescanHash(struct VtFile *file_scan, const char *hash,
	...
	if (ret) VT_ERROR("Adding hash %s\n", hash);
	ret = curl_mime_name(part, "resource");
	if (ret) VT_ERROR("Adding hash multipart name %s\n", hash);
	...
	if (rescan_date) {
		...
		ret = strftime(buff, sizeof(buff) - 1, "%Y%m%d%H%M%S",
		               &time_result);
		part = curl_mime_addpart(mime);
		ret = curl_mime_data(part, buff, CURL_ZERO_TERMINATED);
		if (ret) VT_ERROR("Adding date %s\n", buff);
		ret = curl_mime_name(part, "date");
		if (ret) VT_ERROR("Adding date multipart name %s\n", buff);
	}
	...
	if (period) {
		...
		part = curl_mime_addpart(mime);
		ret = curl_mime_data(part, buff, CURL_ZERO_TERMINATED);
		if (ret) VT_ERROR("Adding period %s\n", buff);
		ret = curl_mime_name(part, "period");
		if (ret) VT_ERROR("Adding period multipart name %s\n", buff);
	}
	...
	if (repeat) {
		...
		part = curl_mime_addpart(mime);
		ret = curl_mime_data(part, buff, CURL_ZERO_TERMINATED);
		if (ret) VT_ERROR("Adding repeat %s\n", buff);
		ret = curl_mime_name(part, "repeat");
		if (ret) VT_ERROR("Adding repeat multipart name %s\n", buff);
	}
	...
	if (notify_url) {
		...
		if (notify_changes_only) {
			...
			if (ret)
				VT_ERROR(
				    "Adding notify_changes_only multipart name "
				    "%s\n",
				    "1");
		}
	}
	...
	if (ret) VT_ERROR("Adding key\n");
	ret = curl_mime_name(part, "apikey");
	if (ret) VT_ERROR("Adding apikey multipart name");
...
int VtFile_rescanDelete(struct VtFile *file_scan, const char *hash) {
	CURL *curl = NULL;
	CURLcode res;
	int ret = 0;
	struct curl_slist *headerlist = NULL;
	...
	curl_easy_setopt(curl, CURLOPT_URL,
	                 VT_API_BASE_URL "file/rescan/delete");
...
int VtFile_search(struct VtFile *file_scan, const char *query,
	...
	if (ret) VT_ERROR("Adding query %s\n", query);
	ret = curl_mime_name(part, "query");
	if (ret) VT_ERROR("Adding query multipart name %s\n", query);
	...
	if (file_scan->offset) {
		part = curl_mime_addpart(mime);
		ret = curl_mime_data(part, file_scan->offset,
		                     CURL_ZERO_TERMINATED);
		if (ret) VT_ERROR("Adding offset\n");
		ret = curl_mime_name(part, "offset");
		if (ret) VT_ERROR("Adding offset multipart name");
	}
	...
	if (resp_json) {
		...
		if (json_is_string(offset_json)) {
			VtFile_setOffset(file_scan,
			                 json_string_value(offset_json));
		}
	}
	...
	if (cb && resp_json) {
		...
		if (offset_json && json_is_string(offset_json) &&
		    json_string_value(offset_json) &&
		    json_string_value(offset_json)[0]) {
			VtFile_setOffset(file_scan,
			                 json_string_value(offset_json));
		}
...
int VtFile_clusters(struct VtFile *file_scan, const char *cluster_date,
	...
	snprintf(url, sizeof(url) - 1,
	         VT_API_BASE_URL "file/clusters?apikey=%s&date=%s",
...
int VtFile_download(struct VtFile *file_scan, const char *hash,
	...
	snprintf(url, sizeof(url) - 1,
	         VT_API_BASE_URL "file/download?apikey=%s&hash=%s",
	...
	curl_easy_setopt(curl, CURLOPT_FOLLOWLOCATION,
	                 1L);  // download API will redirect to link
...
static size_t download_to_file_cb(char *ptr, size_t size, size_t nmemb,
                                  void *userdata) {
...
int VtFile_downloadToFile(struct VtFile *file_scan, const char *hash,
                          const char *out_file) {
