...
struct VtUrlDist;
struct VtResponse;
typedef void(VtUrlDistCb)(const char *url, unsigned long long timestamp,
                          int total, int positives, json_t *raw_json,
                          void *data);
/**
 * @ingroup VtApiPage
 * @defgroup VtUrlDist VtUrlDist URL Distribution service.  Requires private-API
 * with permissions
 * @{
 */
...
/**
 * @brief parse the URL dist results and for each results call the callback
 * function.
 *
