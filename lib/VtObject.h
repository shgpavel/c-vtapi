...
#define VT_OBJECT_LOCK(obj)                                           \
	do {                                                          \
		DBG(3, "LOCKING obj %p\n", obj);                      \
		WaitForSingleObject(&((struct VtObject *)obj)->mutex, \
		                    INFINITE);                        \
		DBG(3, "LOCKED %p\n", obj);                           \
	} while (0)

#define VT_OBJECT_UNLOCK(obj)                                   \
	do {                                                    \
...
/**
 * @struct VtObject_ops
 * @brief VtObject operations, defines various VtObject properties and
 * callbacks.
 */
struct VtObject_ops {
	...
	*/
	int (*obj_compare)(const struct VtObject *dst,
	                   const struct VtObject *src);
