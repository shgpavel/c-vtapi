...
int VtUrlDist_getDistribution(struct VtUrlDist *vt_udist) {
	...
	len = snprintf(get_url, sizeof(get_url) - 1,
	               VT_API_BASE_URL "url/distribution?apikey=%s",
	               vt_udist->api_key);
	if (len < 0) {
		VT_ERROR("sprintf\n");
		goto cleanup;
	...
	if (vt_udist->before) {
		len += ret = snprintf(get_url + len, strlen(get_url) - 1,
		                      "&before=%lld", vt_udist->before);
		if (ret < 0) {
...
int VtUrlDist_parse(struct VtUrlDist *url_dist,
                    void (*cb)(const char *url, unsigned long long timestamp,
                               int total, int positives, json_t *raw_json,
                               void *data),
                    void *user_data) {
	...
	json_array_foreach(resp_json, index, url_obj) {
		...
		if (cb)
			cb(json_string_value(url_str_json),
			   ...
			   (int)json_integer_value(total_json),
			   (int)json_integer_value(positives_json), url_obj,
			   user_data);
	}
