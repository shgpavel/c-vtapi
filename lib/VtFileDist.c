...
void VtFileDist_setAfter(struct VtFileDist *vt_udist,
                         unsigned long long value) {
	vt_udist->after = value;
}

void VtFileDist_setBefore(struct VtFileDist *vt_udist,
                          unsigned long long value) {
...
int VtFileDist_getDistribution(struct VtFileDist *vt_udist) {
	...
	len = snprintf(get_url, 511,
	               VT_API_BASE_URL "file/distribution?apikey=%s",
	               vt_udist->api_key);
	if (len < 0) {
		VT_ERROR("snprintf\n");
		goto cleanup;
	...
	if (vt_udist->before) {
		len += ret =
		    sprintf(get_url + len, "&before=%lld", vt_udist->before);
		if (ret < 0) {
	...
	if (vt_udist->after) {
		len += ret =
		    sprintf(get_url + len, "&after=%lld", vt_udist->after);
		if (ret < 0) {
	...
	if (vt_udist->limit) {
		len += ret =
		    sprintf(get_url + len, "&limit=%d", vt_udist->limit);
		if (ret < 0) {
			VT_ERROR("sprintf after\n");
			goto cleanup;
		}
	...
	if (vt_udist->response)
		VtResponse_put(
		    &vt_udist->response);  // relase reference counter
...
int VtFileDist_parse(struct VtFileDist *url_dist, VtFileDistCb cb,
                     void *user_data) {
	...
	json_array_foreach(resp_json, index, file_jobj) {
		...
		// set the after value, so if we do another query, we will not
		// repeat the same data
		url_dist->after = json_integer_value(timestamp_json);

		// Call user defined callback function
		if (cb)
			cb(json_string_value(url_str_json),
			   json_integer_value(timestamp_json),
			   json_string_value(sha256_json),
			   json_string_value(name_json), file_jobj, user_data);
	}
...
int VtFileDist_process(struct VtFileDist *url_dist,
                       void (*cb)(const char *url, unsigned long long timestamp,
