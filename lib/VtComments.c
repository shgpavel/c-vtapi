...
int VtComments_destructor(struct VtObject *obj) {
	...
	if (vt_comments->before) free(vt_comments->before);
	if (vt_comments->resource) free(vt_comments->resource);
...
int VtComments_add(struct VtComments *vt_comments, const char *comment) {
	...
	if (!vt_comments->resource) {
		VT_ERROR(
		    "Missing Resource. call VtComments_setResource() first\n");
		return -1;
	}
	...
	if (ret) VT_ERROR("Adding resource %s\n", vt_comments->resource);
	...
	if (ret) VT_ERROR("Adding comment %s\n", comment);
	ret = curl_mime_name(part, "comment");
	if (ret) VT_ERROR("Adding comment multipart name %s\n", comment);
	...
	if (vt_comments->response)
		VtResponse_put(
		    &vt_comments->response);  // relase reference counter

	vt_comments->response = VtResponse_new();
	ret =
	    VtResponse_fromJSONstr(vt_comments->response, vt_comments->buffer);
	if (ret) {
...
int VtComments_retrieve(struct VtComments *vt_comments) {
	...
	len = snprintf(get_url, sizeof(get_url) - 1,
	               VT_API_BASE_URL "comments/get?apikey=%s&resource=%s",
	...
	if (vt_comments->before) {
		len += ret =
		    sprintf(get_url + len, "&before=%s", vt_comments->before);
		if (ret < 0) {
