...
int VtResponse_destructor(struct VtObject *obj) {
	...
	if (response->verbose_msg) free(response->verbose_msg);
...
#define VT_RESPONSE_DECLARE_STR_GET(fn_name, member_name)                     \
	char *VtResponse_##fn_name##(struct VtResponse * response, char *buf, \
	                             int buf_siz) {                           \
