...
// #ifdef _MSC_VER
// #define PRINT(FMT,  ...)  printf("%s:%d: " FMT, __FUNCTION__, __LINE__,
// __VA_ARGS__); #else #define PRINT(FMT,ARG...) printf("%s:%d: " FMT,
// __FUNCTION__, __LINE__, ##ARG); #endif

static bool keep_running = true;

void print_usage(const char *prog_name) {
	printf(
	    "%s < --apikey YOUR_API_KEY >   [ --filescan FILE1 ] [ --filescan "
	    "FILE2 ]\n",
	    prog_name);
	printf(
	    "  --apikey <API_KEY>         Your virus total API key. This arg "
	    "1st\n");
	printf(
...
void progress_callback(struct VtFile *file, void *data) {
	...
	printf("progress_callback %lld/%lld\n", (long long)ul_now,
	       (long long)ul_total);
	if (!keep_running) VtFile_cancelOperation(file);
}
...
#define MAX_SCAN_SIZE (32 * 1024 * 1024)
	...
	if (!file_name || !file_name[0]) file_name = "filename";
	...
	while (1) {
		...
		switch (c) {
			...
			case 'c':
				...
				ret =
				    VtFile_clusters(file_scan, optarg,
			...
			case 'd':
				...
				ret = VtFile_downloadToFile(file_scan, optarg,
				                            out);
			...
			case 'f':
				...
				if (ret) {
					...
					response =
					    VtFile_getResponse(file_scan);
					str = VtResponse_toJSONstr(
					    response, VT_JSON_FLAG_INDENT);
					if (str) {
						printf("Response:\n%s\n", str);
						free(str);
					}
			...
			case 'r':
				...
				ret = VtFile_rescanHash(file_scan, optarg, 0, 0,
				                        0, NULL, false);
				// PRINT("rescan ret=%d\n", ret);
				if (ret) {
					printf("Error: %d \n", ret);
			...
			case 'i':
				...
				if (ret) {
					...
					VtResponse_getVerboseMsg(response, buf,
					                         RESP_BUF_SIZE);
					printf("Msg: %s\n", buf);

					ret = VtResponse_getResponseCode(
